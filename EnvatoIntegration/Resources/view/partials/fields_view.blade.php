            ...
            @foreach($custom_fields as $custom_field)
                @if (!$on_create)
                    <div class="cf-group custom-field" id="custom-field-{{ $custom_field->id }}">
                        <div class="text-help">
                           {{ __('Purchase Code') }} @if ($custom_field->required) <i class="required-asterisk"></i>@endif
                        </div>
                @else
                ...
                @endif
                    ...
                    @if ($custom_field->type == EnvatoCustomField::TYPE_DROPDOWN)
                    ...
                    @elseif ($custom_field->type == EnvatoCustomField::TYPE_MULTI_LINE)
                    ...
                    @elseif ($custom_field->type == EnvatoCustomField::TYPE_SINGLE_LINE && !empty($custom_field->options['autosuggest']))
                    ...
                    @else
                        <input name="{{ $custom_field->id }}" class="form-control @if ($custom_field->type == EnvatoCustomField::TYPE_DATE) cf-type-date @endif" value="{{ $custom_field->value }}"
                            @if ($custom_field->type == EnvatoCustomField::TYPE_NUMBER)
                                type="number"
                            @else
                                type="text"
                            @endif
